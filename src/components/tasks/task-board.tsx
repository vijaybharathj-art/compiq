...
import { PriorityPill, TrustBadge } from "@/components/shared/badges";
...
function TaskCard({ task, disabled }: { task: TaskListItem; disabled: boolean }) {
  ...
  // A task with source evidence came from the email pipeline, not a
  // banker (spec §27) — createTaskFromRecommendation() never sets
  // sourceEvidence, so this signal stays reliable for every task on the
  // board, not just pipeline-generated ones.
  const isAutoGenerated = Boolean(task.sourceEvidence);
  ...
  return (
    ...
    >
      ...
      <p className="mt-1.5 text-xs text-muted-foreground line-clamp-2">
        {isAutoGenerated && <span className="font-medium text-foreground/80">Reason: </span>}
        {task.description}
      </p>
      ...
      <div className="mt-2 flex items-center justify-between gap-2">
        ...
        {isAutoGenerated && <TrustBadge confidencePercent={task.aiConfidencePercent} />}
      </div>
      {task.sourceEvidence && (
        <div className="mt-2">
